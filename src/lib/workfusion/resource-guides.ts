...
export const resourceGuides: Record<string, ResourceGuide> = {
  ...
  "fix-mql5-invalid-volume-lot-step": {
    slug: "fix-mql5-invalid-volume-lot-step",
    cluster: "Compiler Fixes",
    pillarSlug: "mql5-compiler-fixer",
    persona: "mq5_developer",
    source: "resource_fix_mql5_invalid_volume_lot_step",
    title: "Fix MQL5 invalid volume and lot step errors",
    metaTitle: "Fix MQL5 Invalid Volume Lot Step | Workfusion",
    description: "Fix MT5 retcode 10014 invalid volume errors caused by 0.00 lots, min/max lot limits, lot step mismatch, or unsafe sizing.",
    searchIntent: "The EA gets retcode 10014, invalid volume, or 0.00 lots when sending a trade request.",
    h1: "Fix MT5 invalid volume before changing the strategy.",
    intro:
    ...
    sections: [
      {
        title: "Read the symbol volume rules",
        body: "Every symbol can have different min lot, max lot, lot step, and directional volume limit. The EA should read these before sending orders.",
        bullets: ["SYMBOL_VOLUME_MIN", "SYMBOL_VOLUME_MAX", "SYMBOL_VOLUME_STEP", "SYMBOL_VOLUME_LIMIT"],
      },
      {
        title: "Normalize then validate",
        body: "Rounding is not enough. Normalize the raw lot to the step, clamp to the allowed range, then reject impossible results like 0.00 lots.",
        bullets: ["Raw calculated lot", "Normalized lot", "Reject if below min", "Reject if risk becomes distorted"],
      },
      {
        title: "Log the sizing chain",
        body: "When the trade fails, print the raw lot, normalized lot, symbol rules, margin result, and retcode so the next fix is based on evidence.",
        bullets: ["Risk money", "Stop distance", "Tick value", "Retcode 10014 context"],
      },
    ],
    checklist: ["Read min/max/step", "Reject 0.00 lots", "Normalize by lot step", "Check margin", "Log raw and normalized lots"],
    cta: "Paste the sizing block into Workfusion Debugger to add lot-step validation and invalid-volume diagnostics.",
    related: ["fixed-risk-lot-sizing-mql5-ea", "fix-invalid-stops-mt5-ea", "prop-firm-ea-readiness-checklist"],
  },
  "fix-mql5-unsupported-filling-mode": {
    slug: "fix-mql5-unsupported-filling-mode",
    cluster: "Compiler Fixes",
    pillarSlug: "mql5-compiler-fixer",
    persona: "mq5_developer",
    source: "resource_fix_mql5_unsupported_filling_mode",
    title: "Fix MQL5 unsupported filling mode",
    metaTitle: "Fix MQL5 Unsupported Filling Mode | Workfusion",
    description: "Fix MT5 retcode 10030 invalid fill errors by selecting an order filling policy supported by the current symbol.",
    searchIntent: "The EA or Python MT5 integration gets unsupported filling mode, invalid fill, or retcode 10030.",
    h1: "Fix unsupported filling mode without guessing FOK, IOC, or RETURN.",
    intro:
    ...
    sections: [
      {
        title: "Let the symbol choose when using CTrade",
        body: "For CTrade workflows, SetTypeFillingBySymbol reads the symbol settings and sets a compatible filling policy.",
        bullets: ["trade.SetTypeFillingBySymbol(_Symbol)", "Log false return", "Avoid blind retries"],
      },
      {
        title: "Read SYMBOL_FILLING_MODE for manual requests",
        body: "For MqlTradeRequest, inspect allowed filling flags and choose a supported policy before OrderSend.",
        bullets: ["SYMBOL_FILLING_MODE", "ORDER_FILLING_FOK", "ORDER_FILLING_IOC", "ORDER_FILLING_RETURN"],
      },
      {
        title: "Compare demo and live conditions",
        body: "A broker can allow different execution policies by account type or symbol suffix. Record the account, symbol, and filling flags.",
        bullets: ["Live versus demo", "Symbol suffix", "Execution mode", "Broker comment"],
      },
    ],
    checklist: ["Avoid hard-coded filling", "Read allowed filling flags", "Use SetTypeFillingBySymbol", "Log selected policy", "Compare demo/live symbol settings"],
    cta: "Paste the trade request into Workfusion Debugger to add filling-mode detection and a safer CTrade setup.",
    related: ["mql5-ctrade-include-trade-object-setup", "expert-advisor-audit-trail", "mql5-logging-diagnostics-ea"],
  },
  "fix-mql5-array-out-of-range-copybuffer": {
    slug: "fix-mql5-array-out-of-range-copybuffer",
    cluster: "Compiler Fixes",
    pillarSlug: "mql5-compiler-fixer",
    persona: "mq5_developer",
    source: "resource_fix_mql5_array_out_of_range_copybuffer",
    title: "Fix MQL5 array out of range after CopyBuffer",
    metaTitle: "Fix MQL5 Array Out of Range CopyBuffer | Workfusion",
    description: "Fix MT5 runtime array out of range errors caused by reading indicator buffers before enough data was copied.",
    searchIntent: "The EA compiles but fails at runtime with array out of range around CopyBuffer or indicator arrays.",
    h1: "Fix CopyBuffer array out of range before trusting indicator signals.",
    intro:
    ...
    sections: [
      {
        title: "Check the copied count exactly",
        body: "Do not only check for -1. If the EA reads buffer[2], CopyBuffer must have copied at least three values.",
        bullets: ["requested count", "copied count", "return early if not enough data"],
      },
      {
        title: "Align array direction",
        body: "ArraySetAsSeries changes how you think about current and previous bars. Confirm whether [0] means current bar in your local arrays.",
        bullets: ["ArraySetAsSeries", "current bar", "previous bar"],
      },
      {
        title: "Log missing data without removing the EA",
        body: "When indicator data is not ready, skip the tick and log a concise reason instead of reading missing indexes.",
        bullets: ["buffer index", "bar shift", "_LastError", "not enough bars"],
      },
    ],
    checklist: ["Check CopyBuffer return count", "Check ArraySize", "Return before reading missing index", "Log _LastError", "Avoid recreating handles on every tick"],
    cta: "Paste the CopyBuffer block into Workfusion Debugger to add count checks and safer indicator reads.",
    related: ["fix-mql5-copybuffer-indicator-handle", "mql5-oninit-ontick-ea-skeleton", "mql5-logging-diagnostics-ea"],
  },
  ...
  "avoid-overfitting-mt5-ea-backtests": {
    slug: "avoid-overfitting-mt5-ea-backtests",
    cluster: "Code Review",
    pillarSlug: "mql5-code-review",
    persona: "quant_researcher",
    source: "resource_avoid_overfitting_mt5_ea_backtests",
    title: "Avoid overfitting MT5 EA backtests",
    metaTitle: "Avoid Overfitting MT5 EA Backtests | Workfusion",
    description: "A practical checklist for detecting overfit EA results, weak trade samples, live/tester mismatch, and missing walk-forward evidence.",
    searchIntent: "The quant or EA builder has strong backtest results but wants to know whether the result is robust enough to continue.",
    h1: "Treat a strong MT5 backtest as a hypothesis, not proof.",
    intro:
    ...
    sections: [
      {
        title: "Check sample and concentration",
        body: "A result carried by a tiny number of trades or one favorable year is not robust evidence.",
        bullets: ["Trade count", "Year-by-year contribution", "Worst segment"],
      },
      {
        title: "Stress the execution assumptions",
        body: "Spread, slippage, commission, tick data, and symbol suffix differences can erase a marginal edge.",
        bullets: ["Spread sensitivity", "Slippage sensitivity", "Broker symbol conditions"],
      },
      {
        title: "Use out-of-sample and walk-forward gates",
        body: "Optimization should produce stable regions, not one magic parameter set. Walk-forward results should not be uniformly negative.",
        bullets: ["Out-of-sample split", "Walk-forward windows", "Parameter stability"],
      },
    ],
    checklist: ["Require enough trades", "Split in-sample and out-of-sample", "Run spread sensitivity", "Check parameter stability", "Archive report and set file"],
    cta: "Paste the EA notes or report summary into Workfusion Backtest Estimate to flag credibility and overfitting risks before the next optimization run.",
    related: ["backtest-failure-triage-mt5-ea", "prepare-ea-for-strategy-tester", "expert-advisor-audit-trail"],
  },
