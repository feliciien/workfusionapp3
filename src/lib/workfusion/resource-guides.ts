...
export const resourceGuides: Record<string, ResourceGuide> = {
  ...
  "fix-mql5-copybuffer-indicator-handle": {
    slug: "fix-mql5-copybuffer-indicator-handle",
    cluster: "Compiler Fixes",
    pillarSlug: "mql5-compiler-fixer",
    persona: "mq5_developer",
    source: "resource_fix_mql5_copybuffer_indicator_handle",
    title: "Fix MQL5 CopyBuffer and indicator handles",
    metaTitle: "Fix MQL5 CopyBuffer Indicator Handles | Workfusion",
    description: "Fix MT5 EA errors caused by using MQL4-style direct indicator calls instead of MQL5 handles and CopyBuffer.",
    searchIntent: "The developer gets wrong-parameter, undeclared, or zero-value indicator errors after moving MT4 indicator logic into MQL5.",
    h1: "Fix MQL5 indicator values with handles and CopyBuffer.",
    intro:
    ...
    sections: [
      {
        title: "Create handles in OnInit",
        body: "Indicator handles should be created once, checked against INVALID_HANDLE, and reused. Recreating handles on every tick makes debugging harder.",
        bullets: ["Create iMA/iRSI/iStochastic handles in OnInit", "Check INVALID_HANDLE", "Release handles in OnDeinit"],
      },
      {
        title: "Read values with CopyBuffer",
        body: "Use CopyBuffer to pull the current and previous values into arrays. Then make signal decisions from those arrays.",
        bullets: ["Copy at least two or three bars", "Check the copied count", "Use ArraySetAsSeries when reading [0] and [1] as current/previous"],
      },
      {
        title: "Separate signal from execution",
        body: "Once indicator values are valid, the EA still needs spread, session, exposure, stop, and retcode checks before trading.",
        bullets: ["Signal function", "Risk gate", "Execution function"],
      },
    ],
    checklist: ["Create handle in OnInit", "Check INVALID_HANDLE", "CopyBuffer returns enough values", "Release handle in OnDeinit", "Log when indicator data is not ready"],
    cta: "Paste the indicator block into Workfusion Debugger to convert MT4-style indicator calls into an MQL5 handle + CopyBuffer flow.",
    related: ["mql4-to-mql5-migration-checklist", "mql5-oninit-ontick-ea-skeleton", "mql5-code-review-before-backtesting"],
  },
  "fix-mql5-history-deal-functions": {
    slug: "fix-mql5-history-deal-functions",
    cluster: "Compiler Fixes",
    pillarSlug: "mql5-compiler-fixer",
    persona: "mq5_developer",
    source: "resource_fix_mql5_history_deal_functions",
    title: "Fix MQL5 history and deal functions",
    metaTitle: "Fix MQL5 History Deal Functions | Workfusion",
    description: "Fix MT5 EA errors around HistoryDeals.mqh, HistoryDealGetTicket, deal properties, magic numbers, and closed-trade PnL.",
    searchIntent: "The developer is trying to read closed deals, TP hits, magic-number PnL, or trade history and gets missing include or undeclared identifier errors.",
    h1: "Fix MQL5 deal history without random HistoryDeals includes.",
    intro:
    ...
    sections: [
      {
        title: "Select the history window first",
        body: "History functions need a selected date range. Without HistorySelect, the EA may see no deals or stale history.",
        bullets: ["HistorySelect(from, to)", "Check false return", "Log GetLastError on failure"],
      },
      {
        title: "Loop deal tickets, not guessed objects",
        body: "Use HistoryDealsTotal and HistoryDealGetTicket, then read properties from each ticket.",
        bullets: ["HistoryDealGetInteger", "HistoryDealGetDouble", "HistoryDealGetString"],
      },
      {
        title: "Group closed PnL correctly",
        body: "For reporting, group by DEAL_MAGIC and DEAL_SYMBOL, and filter entry/exit properties so entries are not double-counted.",
        bullets: ["DEAL_MAGIC", "DEAL_SYMBOL", "DEAL_ENTRY", "DEAL_PROFIT"],
      },
    ],
    checklist: ["Remove duplicate HistoryDeals.mqh files", "Call HistorySelect", "Loop HistoryDealsTotal", "Use HistoryDealGetTicket", "Read properties by ticket", "Group by magic and symbol"],
    cta: "Paste your history/deal code into Workfusion Debugger to get a cleaner MQL5 deal-history workflow.",
    related: ["mql5-logging-diagnostics-ea", "expert-advisor-audit-trail", "mql4-to-mql5-migration-checklist"],
  },
